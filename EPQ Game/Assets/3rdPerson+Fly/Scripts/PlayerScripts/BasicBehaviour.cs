...
public class BasicBehaviour : MonoBehaviour
{
	...
	void Awake ()
	{
		...
		cloneNumber = 0;
		movesRecorder = GameObject.FindGameObjectWithTag("CloneController").GetComponent<PlayerMovementRecorder>();
	}

	void Update()
	{
		//Debug.Log(cloneNumber);
		if (cloneNumber == 0) {
			// Store the input axes.
			h = Input.GetAxis("Horizontal");
			v = Input.GetAxis("Vertical");
			//frames += 1;
			movesRecorder.RecordMoves(h,v);
		} else {
			float[] currentMove = movesRecorder.ReplayMoves(cloneNumber);
			h = currentMove[0];
			v = currentMove[1];
			//Debug.Log(h);
			//Debug.Log(v);
		...
		//Debug.Log("Frames: " + frames.ToString() + " Time: " + timer.ToString());
		//float framesPerSec = frames / timer;
		//Debug.Log("Frames per second: " + framesPerSec.ToString());
